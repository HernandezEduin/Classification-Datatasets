# -*- coding: utf-8 -*-
"""
Created on Thu May  4 20:59:53 2023

@author: Eduin Hernandez
Summary: Benchmark on Training with on MNIST/KMNIST/FashionMNIST
"""
'Libraries'
#Time
import time
...
def parse_args():
    ...
    'Model Details'
    parser.add_argument('--model-name', type=str, default='MLP', help='Model used for the training. Can use LinearRegression, MLP, or LeNet')
    ...
    parser.add_argument('--use-bias', type=str2bool, default='True', help='Whether to use bias vector in model')
    parser.add_argument('--hidden', type=int, default=200, help='Hidden Feature Size of Teacher Network. For MLP model.')
    parser.add_argument('--layers', type=int, default=2, help='Number of Layers in Student Network. For MLP model.')
    ...
    'Simulations'
    parser.add_argument('--simulation-num', type=int, default=1, help='Number of simulations to run')
...
def run_experiment(args):
    ...
    if args.dataset == 'MNIST':
       dataset_train = datasets.MNIST(root=args.dataset_path, train=True, download=True, transform=transformTrain) 
       dataset_test = datasets.MNIST(root=args.dataset_path, train=False, download=True, transform=transformTest)
    elif args.dataset == 'KMNIST':
       dataset_train = datasets.KMNIST(root=args.dataset_path, train=True, download=True, transform=transformTrain) 
       dataset_test = datasets.KMNIST(root=args.dataset_path, train=False, download=True, transform=transformTest)
    elif args.dataset == 'FashionMNIST':
       dataset_train = datasets.FashionMNIST(root=args.dataset_path, train=True, download=True, transform=transformTrain) 
       dataset_test = datasets.FashionMNIST(root=args.dataset_path, train=False, download=True, transform=transformTest)
    else:
        assert False, 'Selected the wrong dataset'
    
    trainloader = torch.utils.data.DataLoader(dataset_train, batch_size=args.batch_size,
                                              shuffle=True)
    testloader = torch.utils.data.DataLoader(dataset_test, batch_size=100,
                                             shuffle=False)
    ...
    if args.verbose_state:
        ...
        print(args.dataset + ' - ' + str(args.model_name) + ' - ' + args.optimizer + ': ' + str(args.learning_rate) + momentum + ' - Start Time: ' + datetime.now().strftime("%H:%M:%S"))
    ...
    for sim_num in range(args.simulation_num):
        ...
        if args.model_name == 'MLP':
            net = sample_models[args.model_name](in_features=784, out_features=10,
                      hidden=args.hidden, layers=args.layers,
                      relu=True, bias=args.use_bias).to(device)
        else:
            net = sample_models[args.model_name](bias=args.use_bias).to(device)
    ...
    if args.verbose_state:
        ...
        print(args.dataset + ' - ' + str(args.model_name) + ' - ' + args.optimizer + ': ' + str(args.learning_rate) + momentum + ' -  End Time: ' + datetime.now().strftime("%H:%M:%S"))
